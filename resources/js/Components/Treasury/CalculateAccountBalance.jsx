...
export default function CalculateAccountBalance({account,rentTotal,purchasesTotal,servicesTotal,recurringTotal,payableTotal,balanceHook}) {
    ...
    useEffect(() => {
        const balanceCalculator = () => {
            setBalance([...balance.filter(x => x.id != account.id), {...balance.find(x => x.id === account.id), calculated: (Number(payableTotal) + Number(rentTotal) + Number(account.treasury_reports[0]?.pivot.account_balance ?? account.starting_balance) - Number(recurringTotal) - Number(purchasesTotal) - Number(servicesTotal)).toFixed(2)}])
        }
        balanceCalculator()
