...
export default function CreateReport({rents, arrears, previousReport, recurringPayments}) {
    ...
    const calculateRecurring = () => {
        return recurringPayments.reduce((a,b) => {
            if(b.frequency === 'weekly') {
                return Number(b.amount) * Number(FrequencyOfWeekDay(b.day_of_month_due))
            } else if (b.frequency === 'monthly') {
                return Number(b.amount) * Number(FrequencyOfDayOfMonth(b.day_of_week_due)) + Number(a)
            } else {
    ...
    function calculateBalance() {
        ...
        return Number(payableTotal) + Number(rentTotal) + Number(previousReport.remaining_budget) - Number(calculateRecurring())
    }
    ...
    console.log(recurringPaymentsToBeMade)
    return (
        <>
        <RangeCalendar 
            value={dates} 
        ...
        <div className="text-xl mt-4">Recurring Payments</div>
            <SmallTable>
                <THead>
                    ...
                    {recurringPaymentsToBeMade?.map((payment, i) => (
                        <Fragment key={payment.id + '.' + payment.occurrence}>
                            <tr>
                                ...
                                <TD>
                                    <input type="file" id="receipt" name="receipt" accept="image/*, .pdf" onChange={(e) => addReceiptToRecurringPayment(e, i)} />
                                </TD>
                            </tr>
                        </Fragment>
                    ))}
